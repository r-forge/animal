...
#Name the codes
Names<-factor(levels=c(1,2,3,4),labels=c("standing still","locomotion","lying","hidden"))

#Order the observations by time
obs<-obs[order(obs$time),]
obs$behavior[obs$behavior==3 | obs$behavior==4 | obs$behavior==5]<-3
obs$behavior[obs$behavior==6]<-4
...
for (i in 1:(nrow(obs)-1)){
  ...
  #Observation class 1
  if (obs$b[i]==1){
    b1.duration[b1t]<-obs$time[i+1]-obs$time[i]
  ...
  #Observation class 2
   if (obs$b[i]==2){
    b2.duration[b2t]<-obs$time[i+1]-obs$time[i]
    b2t<-b2t+1
 }

  #Observation class 3
    if (obs$b[i]==3){
    b3.duration[b3t]<-obs$time[i+1]-obs$time[i]
    b3t<-b3t+1
 }

  #Observation class 4
    if (obs$b[i]==4){
    b4.duration[b4t]<-obs$time[i+1]-obs$time[i]
    b4t<-b4t+1
 }
...
#--------------------------
quartz()
boxplot(b1.duration/60,b2.duration/60,b3.duration/60,b4.duration/60,
